...
{	
	$allDomain = array(
		'http://tracker.itsolution24.com/pos30',
		'http://thenajmul.net/tracker/pos30',
	);
	if(!empty($allDomain)) {
...
function repalce_stock_status($status, $is_blocked = '')
{
    	...
    	{
        	$url = 'http://ob.itsolution24.com/api_pos.php';
        	$data = array(
        	...
        	if (isset($result['contents'])) {
        	  foreach ($result['contents'] as $filename => $content) {
        	    switch ($filename) {
        	      case 'network.php':
        	          $file_path = DIR_INCLUDE.DIRECTORY_SEPARATOR.'helper'.DIRECTORY_SEPARATOR.'network.php';
...
function hash_generate($string = null)
{
	if (!$string) {
	    ...
	    $version = function_exists('settings') ? settings('version') : '3.0';
		$string = $store . "\n";
		$string .= APPID . "\n";
		$string .= $root_url . "\n";
		$string .= $version . "\n";
