...
function (
    ...
    $scope._calcTotalPayable = function ($childScope) {
        ...
        console.log("total payable => ", $scope.payable);
        console.log("shippingPercentage => ",shippingPercentage);
        console.log("othersCharge => ", $scope.othersCharge);
        console.log("dueAmount => ",$scope.dueAmount);
        console.log("discountPercentage => ",discountPercentage);

        $scope.payable = (parseFloat($scope.payable) + shippingPercentage + $scope.othersCharge + $scope.dueAmount) - discountPercentage;
        ...
        then(function(response) {
            if (response.data.p_id) {
                ...
                if (find) {
                    window._.map($scope.itemArray, function (item) {
                        if (item.id == response.data.p_id) {
                            ...
                            if ((qty > response.data.quantity_in_stock || $scope.itemQuantity >= response.data.quantity_in_stock) && response.data.p_type != 'service') {
                                ...
                                window.toastr.error("No puedes vender asi!", "Warning!");
                                return false;
                            }
                            if (window.store.sound_effect == 1) {
                            ...
                            item.price = (item.price + (parseFloat(response.data.sell_price) * qty)) + taxamount;
                            item.quantity = item.price / item.subTotal;    
                            //$("#item_price_"+item.id).trigger("keyup");
                    ...
                    //item.subTotal = parseFloat(response.data.sell_price);
                    $scope.totalQuantity = $scope.totalQuantity + qty;
    ...
    $scope.removeItemFromInvoice = function (index, id) {
        ...
        window._.map($scope.itemArray, function (item, key) {
            if (item.id == id) {
                console.log("current total quantity => ", $scope.totalQuantity);
    ...
    $scope.popupWindow = function(data) {
        ...
        mywindow.document.write('<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><title>Imprimir</title>');
    ...
    $(document).delegate(".item_quantity", "keyup", function(e) {
        ...
        console.log("item qunatity => ", itemquantity);
        var totalAmount = 0;
        window._.map($scope.itemArray, function (item) {
            if (item.id == itemid) {
